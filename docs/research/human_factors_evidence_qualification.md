...
- **Zero Production PASS/FAIL Thresholds Added in this Phase**: Qualified external records are architectural and research candidates. No new runtime PASS/FAIL rules or threshold scaling algorithms are activated in this phase.
...
$$\text{ReferenceEnvelope} = \left\{ \text{Governing}, \text{Recommended}, \text{Optimal}, \text{Secondary}, \text{Adapted}, \text{Conservative}, \text{Descriptive} \right\}$$
...
- **Document Nature**: Non-binding **Human Factors Design Guidance** (NOT a statutory Federal Motor Vehicle Safety Standard or regulation).
- **Target Scope**: In-vehicle electronic interfaces for passenger car drivers under moving vehicle conditions.
- **Viewing-Distance Context**: Document guidance indicates nominal viewing distances of $D = 0.5 - 1.1\text{ m}$ for the equations in this section (treated as source calculation context, NOT a prefilled default in product inputs).
- **Evidence Records**:
...
- **Current Edition Status**: **ISO 15008:2017** is the published, active standard. (Edition 4 revision **ISO/CD 15008** is currently under development at stage 30.99 CD approved for registration as DIS, and must NOT be treated as a published standard or imported into production evidence).
...
- **Critical Boundary**: The visual-angle definition of CSS px provides coordinate conversion context, but does **NOT** make WCAG SC 2.5.8 (Target Size 24px) visual-angle transferable, because SC 2.5.8 addresses `motor_target_acquisition`.
...
| **`REF-APPLE-BODY-TEXT`** | Apple HIG Dynamic Type | `visual_legibility` | `character_height` | `secondary_reference` (17pt nominal) | Mobile / Non-Critical Cross-Domain | `verified` (Source) | Qualified Platform Guidance | Secondary design comparison |
| **`REF-ANDROID-BODY-TEXT`** | Material Design | `visual_legibility` | `character_height` | `secondary_reference` (16sp nominal) | Android / Non-Critical Cross-Domain | `verified` (Source) | Qualified Platform Guidance | Secondary design comparison |
...
- If the current measurement only provides `element_visual_bounds` (the entire text container bounding box including line padding):
  $$\text{measurement\_matched} = \text{false}$$
  $$\text{assigned\_role} = \text{descriptive\_only}$$
